...
class PartyRoomManager {
    ...
    for (const room of this.rooms.values()) {
      for (const seat of room.players) {
        ...
        if (seat.voiceConnected) {
          ...
          this.io?.to(getSocketRoom(room.roomNo)).emit("voice:user-left", {
            roomNo: room.roomNo,
            userId: seat.userId
          });
        }
  ...
  voiceJoin(roomNo, userId, muted = false) {
    ...
    for (const socketId of seat.socketIds) {
      this.io?.to(socketId).emit("voice:peers", {
        roomNo,
        peers
      });
    }
    ...
    if (!alreadyJoined) {
      this.io?.to(getSocketRoom(roomNo)).emit("voice:user-joined", {
        roomNo,
        userId: seat.userId,
  ...
  voiceLeave(roomNo, userId) {
    ...
    seat.voiceConnected = false;
    this.io?.to(getSocketRoom(roomNo)).emit("voice:user-left", {
      roomNo,
      userId: seat.userId
    });
    this.emitRoom(room);
  ...
  updateVoiceState(roomNo, userId, muted) {
    ...
    seat.voiceMuted = Boolean(muted);
    this.io?.to(getSocketRoom(roomNo)).emit("voice:state", {
      roomNo,
      userId: seat.userId,
      muted: seat.voiceMuted
    });
  ...
  relayVoiceSignal(roomNo, userId, targetUserId, data) {
    ...
    for (const socketId of targetSeat.socketIds) {
      this.io?.to(socketId).emit("voice:signal", {
        roomNo,
        fromUserId: userId,
        data
      });
  ...
  emitRoom(room, meta = {}) {
    for (const player of room.players) {
      for (const socketId of player.socketIds) {
        this.io?.to(socketId).emit("party:update", {
