...
const {
  getRoomExpiryMs: resolveRoomExpiryMs,
...
const VOICE_REPORT_REASON_CODES = Object.freeze({
  [VOICE_REPORT_REASONS.startupTimeout]: "voice-startup-timeout",
  [VOICE_REPORT_REASONS.iceFailed]: "voice-ice-failed",
  [VOICE_REPORT_REASONS.persistentDisconnect]: "voice-persistent-disconnect"
});

function cloneIceServers(iceServers = []) {
  return (Array.isArray(iceServers) ? iceServers : []).map((entry) => ({
    ...entry,
    ...(Array.isArray(entry?.urls) ? { urls: [...entry.urls] } : {})
  }));
}
...
function normalizePositiveIntegerWithFallback(value, fallback) {
...
function normalizeVoiceTransportMode(value) {
  return value === VOICE_TRANSPORT_MODES.relayRequired
    ? VOICE_TRANSPORT_MODES.relayRequired
    : VOICE_TRANSPORT_MODES.directPreferred;
}

function normalizeVoiceRuntimeState(value) {
  if (value === "blocked" || value === "degraded") {
    return value;
  }
...
function createDefaultRuntimeVoiceState() {
  return {
    state: "healthy",
    reasonCode: "",
    message: "",
...
function normalizeRuntimeVoiceState(value = {}) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return {
    state: normalizeVoiceRuntimeState(source.state || source.runtimeState),
    reasonCode: String(source.reasonCode || "").trim(),
    message: String(source.message || "").trim(),
    safeActions: Array.isArray(source.safeActions)
      ? source.safeActions
...
function createDefaultVoiceRecoveryState() {
  return {
    autoResumeEligible: false,
    resumeMuted: true,
    rejoinBy: null,
    lastMode: VOICE_TRANSPORT_MODES.directPreferred
  };
}

function normalizeVoiceRecoveryState(value = {}, defaultMode = VOICE_TRANSPORT_MODES.directPreferred) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const rejoinBy = normalizeReconnectGraceEndsAt(source.rejoinBy);
  const autoResumeEligible =
    Boolean(source.autoResumeEligible) &&
    Boolean(rejoinBy) &&
    Date.parse(rejoinBy) > Date.now();

  return {
    autoResumeEligible,
    resumeMuted: source.resumeMuted !== false,
    rejoinBy: autoResumeEligible ? rejoinBy : null,
    lastMode: normalizeVoiceTransportMode(source.lastMode || defaultMode)
  };
}

function buildDefaultVoiceTransportState(config = getPartyVoiceTransportConfig()) {
  return {
    mode: VOICE_TRANSPORT_MODES.directPreferred,
    stickyRelay: Boolean(config.stickyRelay),
    startupProbeMs: Number(config.startupProbeMs),
    persistentFailureMs: Number(config.persistentFailureMs),
    reconnectGraceSeconds: Number(config.reconnectGraceSeconds),
    resumeMutedOnRecovery: Boolean(config.resumeMutedOnRecovery),
...
function normalizeVoiceTransportState(value = {}, config = getPartyVoiceTransportConfig()) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return {
    mode: normalizeVoiceTransportMode(source.mode),
    stickyRelay: normalizeBooleanWithFallback(source.stickyRelay, config.stickyRelay),
    startupProbeMs: normalizePositiveIntegerWithFallback(source.startupProbeMs, config.startupProbeMs),
    persistentFailureMs: normalizePositiveIntegerWithFallback(
      source.persistentFailureMs,
      config.persistentFailureMs
    ),
    reconnectGraceSeconds: normalizePositiveIntegerWithFallback(
      source.reconnectGraceSeconds,
      config.reconnectGraceSeconds
    ),
    resumeMutedOnRecovery: normalizeBooleanWithFallback(
      source.resumeMutedOnRecovery,
      config.resumeMutedOnRecovery
    ),
    iceServers:
      Array.isArray(source.iceServers) && source.iceServers.length > 0
        ? cloneIceServers(source.iceServers)
        : cloneIceServers(config.iceServers),
    runtimeState: normalizeVoiceRuntimeState(source.runtimeState),
    lastReasonCode: String(source.lastReasonCode || "").trim(),
    lastTransitionAt: normalizeReconnectGraceEndsAt(source.lastTransitionAt)
  };
}

function buildVoiceDiagnostics(room) {
  const voiceTransport = normalizeVoiceTransportState(
    {
      ...buildDefaultVoiceTransportState(),
      ...room?.voiceTransport
    },
    getPartyVoiceTransportConfig()
  );
  ...
  return {
    mode: voiceTransport.mode,
    stickyRelay: voiceTransport.stickyRelay,
    runtimeState: voiceTransport.runtimeState,
    lastReasonCode: voiceTransport.lastReasonCode,
    lastTransitionAt: voiceTransport.lastTransitionAt,
    lastRecoveredAt: normalizeReconnectGraceEndsAt(room?.lastVoiceRecoveredAt),
    resumeMutedOnRecovery: voiceTransport.resumeMutedOnRecovery,
    reconnectGraceSeconds: voiceTransport.reconnectGraceSeconds
  };
}
...
class PartyRoomManager {
  ...
  createRoom(owner, gameKey, overrides = {}) {
    ...
    const room = {
      ...
      feed: [],
      lastResult: null,
      runtimeVoiceState: createDefaultRuntimeVoiceState(),
      voiceTransport: buildDefaultVoiceTransportState(),
      lastVoiceRecoveredAt: null,
  ...
  registerSocket(roomNo, userId, socket) {
    ...
    seat.voiceRecovery = normalizeVoiceRecoveryState(seat.voiceRecovery, room.voiceTransport?.mode);
    ...
    for (const room of this.rooms.values()) {
      for (const seat of room.players) {
        ...
        if (seat.voiceConnected) {
          seat.voiceRecovery = normalizeVoiceRecoveryState(
            {
              autoResumeEligible: true,
              resumeMuted: true,
              rejoinBy: seat.reconnectGraceEndsAt,
              lastMode: room.voiceTransport?.mode
            },
            room.voiceTransport?.mode
          );
          seat.voiceConnected = false;
          seat.voiceMuted = true;
  ...
  voiceJoin(roomNo, userId, muted = false) {
    ...
    if (room.config?.voiceEnabled === false) {
      throw new Error("房间未开启语音");
    }

    const voiceAvailability = buildAvailabilityEnvelope({
      controls: getAvailabilityControlsSync(),
      familyKey: "party",
      gameKey: room.gameKey,
      roomAvailability: this.getAdminAvailability(room),
      supportsVoice: true,
      runtimeVoiceState: room.runtimeVoiceState
    }).subsystems.voice;
    if (voiceAvailability?.state === "blocked") {
      throw new Error(voiceAvailability.message || "語音暫停");
    }

    const alreadyJoined = seat.voiceConnected;
    const recoveredMutedJoin =
      Boolean(seat.voiceRecovery?.autoResumeEligible) &&
      seat.voiceRecovery?.resumeMuted !== false &&
      Boolean(muted);
    seat.voiceConnected = true;
    seat.voiceMuted = Boolean(muted);
    seat.voiceRecovery = normalizeVoiceRecoveryState(
      createDefaultVoiceRecoveryState(),
      room.voiceTransport?.mode
    );
    if (recoveredMutedJoin) {
      room.lastVoiceRecoveredAt = new Date().toISOString();
    }
  ...
  reportVoiceTransport(roomNo, userId, report = {}) {
    const room = this.assertRoom(roomNo);
    const seat = this.findSeat(room, userId);
    if (!seat) {
      throw new Error("不在房间内");
    ...
    const reason = String(report.reason || report.reasonCode || "").trim();
    const mappedReasonCode = VOICE_REPORT_REASON_CODES[reason];
    if (!mappedReasonCode) {
      return room;
    }

    const transitionedAt = new Date().toISOString();
    room.runtimeVoiceState = normalizeRuntimeVoiceState({
      state: "degraded",
      reasonCode: mappedReasonCode,
      message: "",
      safeActions: []
    });
    room.voiceTransport = normalizeVoiceTransportState(
      {
        ...room.voiceTransport,
        mode: VOICE_TRANSPORT_MODES.relayRequired,
        stickyRelay: true,
        runtimeState: "degraded",
        lastReasonCode: mappedReasonCode,
  ...
  serializeRoom(room, viewerUserId = null) {
    room.runtimeVoiceState = normalizeRuntimeVoiceState(room.runtimeVoiceState);
    room.voiceTransport = normalizeVoiceTransportState(
      {
        ...buildDefaultVoiceTransportState(),
        ...room.voiceTransport,
        runtimeState: room.runtimeVoiceState.state
      },
      getPartyVoiceTransportConfig()
    );
    for (const player of room.players) {
      player.voiceRecovery = normalizeVoiceRecoveryState(
        player.voiceRecovery,
        room.voiceTransport.mode
      );
    }
    ...
    return {
      ...
      voiceTransport: {
        ...room.voiceTransport,
        iceServers: cloneIceServers(room.voiceTransport.iceServers)
      },
      voiceDiagnostics: buildVoiceDiagnostics(room),
      ...
      viewer: viewerSeat
        ? {
            ...
            voiceRecovery: normalizeVoiceRecoveryState(
              viewerSeat.voiceRecovery,
              room.voiceTransport.mode
            ),
  ...
  createHumanSeat(user, seatIndex) {
    return {
      ...
      voiceMuted: true,
      voiceRecovery: createDefaultVoiceRecoveryState(),
      socketIds: new Set()
    };
  }
  ...
  getReconnectGraceMs() {
    if (Number.isFinite(this.reconnectGraceMs) && this.reconnectGraceMs > 0) {
      return this.reconnectGraceMs;
    }

    const configReconnectGraceSeconds = Number(getPartyVoiceTransportConfig().reconnectGraceSeconds);
    if (Number.isInteger(configReconnectGraceSeconds) && configReconnectGraceSeconds > 0) {
      return configReconnectGraceSeconds * 1000;
    }
  ...
  scheduleReconnectExpiry(roomNo, userId) {
    ...
    const timer = setTimeout(() => {
      ...
      liveSeat.voiceRecovery = normalizeVoiceRecoveryState(
        {
          ...liveSeat.voiceRecovery,
          autoResumeEligible: false,
          rejoinBy: null
        },
        liveRoom.voiceTransport?.mode
      );
