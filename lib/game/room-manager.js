...
class RoomManager {
  ...
  createRoom(owner, template, overrides = {}) {
    const roomNo = this.generateRoomNo();
  ...
  submitBid(roomNo, userId, value) {
    ...
    if (value === 3 || room.round.bidTurns >= room.players.length) {
  ...
  async finishGame(room, winnerSeat) {
    ...
    const spring = this.detectSpring(room);
    if (spring) {
  ...
  performAutomatedAction(room, seat) {
    ...
    if (room.round.stage === "bidding") {
      ...
      const bidValue = Math.max(
        room.round.highestBid + 1 > 3 ? 0 : room.round.highestBid + 1,
        chooseBid(hand)
      );
      const finalBid =
        room.round.highestBid >= 3
          ? 0
          : Math.min(3, Math.max(room.round.highestBid + 1, bidValue));

      try {
        this.submitBid(room.roomNo, seat.userId, finalBid > 3 ? 0 : finalBid);
      } catch (error) {
    ...
    if (room.round.stage === "playing") {
      ...
      const lastPlay =
        room.round.lastPlay && room.round.lastPlay.seatIndex !== seat.seatIndex
          ...
          : null;
      const play = choosePlay(hand, lastPlay);
      ...
      if (play) {
        ...
        const fallback = choosePlay(hand, null);
        if (fallback) {
          this.submitPlay(
            room.roomNo,
            seat.userId,
  ...
  emitRoom(room, meta = {}) {
    for (const player of room.players) {
      ...
      for (const socketId of player.socketIds) {
        this.io?.to(socketId).emit("room:update", {
...
function pickAllowedOverrides(input) {
  ...
  for (const key of [
    ...
    if (input[key] !== undefined && input[key] !== null && input[key] !== "") {
      overrides[key] =
        typeof input[key] === "string" && !Number.isNaN(Number(input[key]))
          ? Number(input[key])
          : input[key];
    }
  }
...
function normalizeRoomSettings(settings) {
  ...
  normalized.baseScore = Math.max(10, Number(normalized.baseScore || 50));
  normalized.countdownSeconds = Math.max(8, Number(normalized.countdownSeconds || 18));
  normalized.autoTrusteeMinSeconds = Math.max(
    1,
    Number(normalized.autoTrusteeMinSeconds || 2)
  );
  normalized.autoTrusteeMaxSeconds = Math.max(
    normalized.autoTrusteeMinSeconds,
    Number(
      normalized.autoTrusteeMaxSeconds ||
        normalized.autoTrusteeSeconds ||
        normalized.autoTrusteeMinSeconds + 3
    )
  );
  normalized.roomVisibility =
    normalized.roomVisibility === "private" ? "private" : "public";
