...
const { buildSeatRecoveryState } = require("../shared/network-contract");
const {
...
function getRoomProviders() {
  return [
    ...
    },
    {
      ...
      resolveGameKey: () => "pickred"
    },
    {
      familyKey: "card",
      manager: getBigTwoRoomManager(),
      resolveGameKey: () => "bigtwo"
    },
    {
      familyKey: "card",
      manager: getMahjongRoomManager(),
      resolveGameKey: () => "mahjong"
    },
    {
      familyKey: "party",
...
function buildRoomVoiceDiagnostics(provider, room) {
  if (provider.familyKey !== "party") {
    return null;
  }

  const serializedRoom = provider.manager.serializeRoom(room);
  const degradedVoiceState = serializedRoom?.degradedState?.subsystems?.voice || null;
  const diagnostics = serializedRoom?.voiceDiagnostics || null;
  if (!diagnostics && !degradedVoiceState) {
    return null;
  }

  return {
    ...(diagnostics || {}),
    degradedState: degradedVoiceState
      ? {
          state: degradedVoiceState.state,
          reasonCode: degradedVoiceState.reasonCode,
          message: degradedVoiceState.message,
          safeActions: [...(degradedVoiceState.safeActions || [])]
        }
      : null
  };
}
...
function serializeLiveRoomDetail(match) {
  ...
  return {
    ...
    voiceDiagnostics: buildRoomVoiceDiagnostics(match.provider, match.room),
    occupants,
    actions: {
      inspect: true,
...
function serializeSnapshotRoomDetail(entry) {
  ...
  return {
    ...
    occupants: occupantIds.map((occupantId, index) => ({
      ...
      seatIndex: index,
      isBot: false,
      isOwner: String(occupantId) === String(entry.ownerId),
      ready: false,
...
function buildOccupants(players = [], ownerId, options = {}) {
  ...
  return players.map((player) => {
    const recoveryState = buildSeatRecoveryState({
    ...
    return {
      ...
      presenceState: player.presenceState || recoveryState.presenceState,
      canRemove
    };
  });
}
