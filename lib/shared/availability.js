...
function applyVoiceGameOverrides(status, options = {}) {
  if (
    status?.subsystem !== AVAILABILITY_SUBSYSTEMS.VOICE ||
    String(options.gameKey || "").trim() !== "undercover"
  ) {
    return status;
  }
  ...
  const genericSafeActions = DEFAULT_SAFE_ACTIONS[status.subsystem]?.[status.state] || [];
...
function resolveAvailabilityStatus(controls, options = {}) {
  ...
  return applyVoiceGameOverrides(status, { familyKey, gameKey });
}

function normalizeRuntimeVoiceState(input = {}, options = {}) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : null;
  if (!source) {
    return null;
  }

  const state = normalizeAvailabilityState(source.state || source.runtimeState);
  const safeActions = normalizeSafeActions(
    source.safeActions,
    AVAILABILITY_SUBSYSTEMS.VOICE,
    state
  );
  ...
  return {
    ...
    scope: "runtime",
    familyKey: "",
    configured: Object.keys(source).length > 0
  };
}

function mergeVoiceAvailabilityStatus(baseStatus, runtimeVoiceState, options = {}) {
  if (!runtimeVoiceState || baseStatus.state === AVAILABILITY_STATES.BLOCKED) {
    return applyVoiceGameOverrides(baseStatus, options);
  }

  const runtimeStatus = normalizeRuntimeVoiceState(runtimeVoiceState, options);
  if (!runtimeStatus) {
    return applyVoiceGameOverrides(baseStatus, options);
  }

  if (
    (AVAILABILITY_STATE_ORDER[runtimeStatus.state] || 0) <=
    (AVAILABILITY_STATE_ORDER[baseStatus.state] || 0)
  ) {
    return applyVoiceGameOverrides(baseStatus, options);
  }

  return applyVoiceGameOverrides(
    {
      ...runtimeStatus,
      configured: runtimeStatus.configured || baseStatus.configured
    },
    options
  );
}
...
function buildAvailabilityEnvelope(options = {}) {
  ...
  const voice = supportsVoice
    ? mergeVoiceAvailabilityStatus(
      resolveAvailabilityStatus(options.controls, {
        familyKey,
        gameKey,
        subsystem: AVAILABILITY_SUBSYSTEMS.VOICE
      }),
      options.runtimeVoiceState,
      {
        familyKey,
        gameKey
      }
