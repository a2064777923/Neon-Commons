const AUTH_SCOPES = Object.freeze({
  PUBLIC: "public",
  USER: "user",
  ADMIN: "admin"
});

const PRESENCE_STATES = Object.freeze({
  CONNECTED: "connected",
  RECONNECTING: "reconnecting",
  DISCONNECTED: "disconnected"
});

const RECOVERY_SCOPES = Object.freeze({
  ACCOUNT: "account",
  ROOM: "room"
});

const VOICE_TRANSPORT_MODES = Object.freeze({
  directPreferred: "direct-preferred",
  relayRequired: "relay-required"
});

const VOICE_REPORT_REASONS = Object.freeze({
  startupTimeout: "startup-timeout",
  persistentDisconnect: "persistent-disconnect",
  iceFailed: "ice-failed",
  recovered: "recovered"
});

const LAUNCH_CONTRACT_KINDS = Object.freeze({
  DIRECT: "direct",
  FAMILY_LOBBY: "family-lobby",
  TITLE_LOBBY: "title-lobby",
  ROOM_ENTRY_ONLY: "room-entry-only",
  METADATA_ONLY: "metadata-only"
});

const API_ROUTE_PATTERNS = Object.freeze({
  auth: Object.freeze({
    login: "/api/auth/login",
    logout: "/api/auth/logout",
    register: "/api/auth/register"
  }),
  hub: "/api/hub",
  me: "/api/me",
  profile: "/api/profile",
  leaderboard: "/api/leaderboard",
  templates: "/api/templates",
  cardRooms: Object.freeze({
    list: "/api/rooms",
    detail: "/api/rooms/:roomNo",
    join: "/api/rooms/:roomNo/join"
  }),
  partyRooms: Object.freeze({
    list: "/api/party/rooms",
    detail: "/api/party/rooms/:roomNo",
    join: "/api/party/rooms/:roomNo/join"
  }),
  boardRooms: Object.freeze({
    list: "/api/board/rooms",
    detail: "/api/board/rooms/:roomNo",
    join: "/api/board/rooms/:roomNo/join"
  }),
  pickredRooms: Object.freeze({
    list: "/api/pickred/rooms",
    detail: "/api/pickred/rooms/:roomNo",
    join: "/api/pickred/rooms/:roomNo/join"
  }),
  bigtwoRooms: Object.freeze({
    list: "/api/bigtwo/rooms",
    detail: "/api/bigtwo/rooms/:roomNo",
    join: "/api/bigtwo/rooms/:roomNo/join"
  }),
  mahjongRooms: Object.freeze({
    list: "/api/mahjong/rooms",
    detail: "/api/mahjong/rooms/:roomNo",
    join: "/api/mahjong/rooms/:roomNo/join"
  }),
  racingRooms: Object.freeze({
    list: "/api/racing/rooms",
    detail: "/api/racing/rooms/:roomNo",
    join: "/api/racing/rooms/:roomNo/join"
  }),
  fightingRooms: Object.freeze({
    list: "/api/fighting/rooms",
    detail: "/api/fighting/rooms/:roomNo",
    join: "/api/fighting/rooms/:roomNo/join"
  }),
  roomEntry: Object.freeze({
    resolve: "/api/room-entry/resolve",
    shareable: "/api/room-entry/shareable",
    guest: "/api/room-entry/guest",
    guestSync: "/api/room-entry/guest-sync"
  }),
  admin: Object.freeze({
    players: "/api/admin/players",
    playerAdjust: "/api/admin/players/:id/adjust",
    templates: "/api/admin/templates",
    config: "/api/admin/config",
    capabilities: "/api/admin/capabilities",
    runtime: "/api/admin/runtime",
    logs: "/api/admin/logs",
    liveRooms: Object.freeze({
      list: "/api/admin/live-rooms",
      detail: "/api/admin/live-rooms/:roomNo",
      action: "/api/admin/live-rooms/:roomNo/actions",
      occupantRemove: "/api/admin/live-rooms/:roomNo/occupants/:occupantId/remove"
    })
  })
});

function buildPath(pattern, params = {}, query = {}) {
  const path = pattern.replace(/:([A-Za-z0-9_]+)/g, (_match, name) => {
    if (!(name in params)) {
      throw new Error(`Missing route param: ${name}`);
    }

    return encodeURIComponent(String(params[name]));
  });

  const entries = Object.entries(query).filter(([, value]) => value !== undefined && value !== null && value !== "");
  if (entries.length === 0) {
    return path;
  }

  const search = entries
    .map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(String(value))}`)
    .join("&");

  return `${path}?${search}`;
}

function createHandlerContract(id, path, methods, auth) {
  const normalizedMethods = [...methods].map((method) => String(method).toUpperCase());
  const normalizedAuth = normalizeAuth(normalizedMethods, auth);

  return Object.freeze({
    id,
    path,
    methods: Object.freeze(normalizedMethods),
    auth: normalizedAuth
  });
}

function normalizeAuth(methods, auth) {
  if (typeof auth === "string") {
    return Object.freeze(
      Object.fromEntries(methods.map((method) => [method, auth]))
    );
  }

  return Object.freeze(
    Object.fromEntries(
      methods.map((method) => [
        method,
        auth?.[method] || auth?.[method.toLowerCase()] || auth?.all || null
      ])
    )
  );
}

function getContractAuthScope(contract, method) {
  if (!contract || !contract.auth) {
    return null;
  }

  return contract.auth[String(method || "").toUpperCase()] || null;
}

function normalizeReconnectGraceEndsAt(value) {
  if (!value) {
    return null;
  }

  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) {
    return null;
  }

  return parsed.toISOString();
}

function buildPresenceState(input = {}, options = {}) {
  if (input.connected) {
    return PRESENCE_STATES.CONNECTED;
  }

  const reconnectGraceEndsAt = normalizeReconnectGraceEndsAt(input.reconnectGraceEndsAt);
  if (!reconnectGraceEndsAt) {
    return PRESENCE_STATES.DISCONNECTED;
  }

  const now = Number.isFinite(options.now) ? options.now : Date.now();
  return Date.parse(reconnectGraceEndsAt) > now
    ? PRESENCE_STATES.RECONNECTING
    : PRESENCE_STATES.DISCONNECTED;
}

function buildSeatRecoveryState(input = {}, options = {}) {
  const connected = Boolean(input.connected);
  const reconnectGraceEndsAt = normalizeReconnectGraceEndsAt(input.reconnectGraceEndsAt);

  return {
    connected,
    presenceState: buildPresenceState(
      {
        connected,
        reconnectGraceEndsAt
      },
      options
    ),
    recoveryEligible: !Boolean(input.isBot),
    reconnectGraceEndsAt
  };
}

function buildSessionRecoveryState(session, options = {}) {
  if (!session) {
    return null;
  }

  return {
    presenceState: PRESENCE_STATES.CONNECTED,
    recoveryEligible:
      session.kind === "user" ||
      (session.kind === "guest" && Boolean(session.roomNo && session.gameKey)),
    reconnectGraceEndsAt: normalizeReconnectGraceEndsAt(options.reconnectGraceEndsAt),
    recoveryScope: session.kind === "guest" ? RECOVERY_SCOPES.ROOM : RECOVERY_SCOPES.ACCOUNT
  };
}

const API_ROUTES = Object.freeze({
  auth: Object.freeze({
    login: () => API_ROUTE_PATTERNS.auth.login,
    logout: () => API_ROUTE_PATTERNS.auth.logout,
    register: () => API_ROUTE_PATTERNS.auth.register
  }),
  hub: () => API_ROUTE_PATTERNS.hub,
  me: () => API_ROUTE_PATTERNS.me,
  profile: () => API_ROUTE_PATTERNS.profile,
  leaderboard: () => API_ROUTE_PATTERNS.leaderboard,
  templates: () => API_ROUTE_PATTERNS.templates,
  cardRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.cardRooms.list,
    create: () => API_ROUTE_PATTERNS.cardRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.cardRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.cardRooms.join, { roomNo })
  }),
  partyRooms: Object.freeze({
    list: (gameKey) => buildPath(API_ROUTE_PATTERNS.partyRooms.list, {}, gameKey ? { gameKey } : {}),
    create: (gameKey) => buildPath(API_ROUTE_PATTERNS.partyRooms.list, {}, gameKey ? { gameKey } : {}),
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.partyRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.partyRooms.join, { roomNo })
  }),
  boardRooms: Object.freeze({
    list: (gameKey) => buildPath(API_ROUTE_PATTERNS.boardRooms.list, {}, gameKey ? { gameKey } : {}),
    create: (gameKey) => buildPath(API_ROUTE_PATTERNS.boardRooms.list, {}, gameKey ? { gameKey } : {}),
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.boardRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.boardRooms.join, { roomNo })
  }),
  pickredRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.pickredRooms.list,
    create: () => API_ROUTE_PATTERNS.pickredRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.pickredRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.pickredRooms.join, { roomNo })
  }),
  bigtwoRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.bigtwoRooms.list,
    create: () => API_ROUTE_PATTERNS.bigtwoRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.bigtwoRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.bigtwoRooms.join, { roomNo })
  }),
  mahjongRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.mahjongRooms.list,
    create: () => API_ROUTE_PATTERNS.mahjongRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.mahjongRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.mahjongRooms.join, { roomNo })
  }),
  racingRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.racingRooms.list,
    create: () => API_ROUTE_PATTERNS.racingRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.racingRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.racingRooms.join, { roomNo })
  }),
  fightingRooms: Object.freeze({
    list: () => API_ROUTE_PATTERNS.fightingRooms.list,
    create: () => API_ROUTE_PATTERNS.fightingRooms.list,
    detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.fightingRooms.detail, { roomNo }),
    join: (roomNo) => buildPath(API_ROUTE_PATTERNS.fightingRooms.join, { roomNo })
  }),
  roomEntry: Object.freeze({
    resolve: (roomNo, gameKeyHint) =>
      buildPath(API_ROUTE_PATTERNS.roomEntry.resolve, {}, {
        roomNo,
        ...(gameKeyHint ? { gameKeyHint } : {})
      }),
    shareable: () => API_ROUTE_PATTERNS.roomEntry.shareable,
    guest: () => API_ROUTE_PATTERNS.roomEntry.guest,
    guestSync: () => API_ROUTE_PATTERNS.roomEntry.guestSync
  }),
  admin: Object.freeze({
    players: () => API_ROUTE_PATTERNS.admin.players,
    playerAdjust: (id) => buildPath(API_ROUTE_PATTERNS.admin.playerAdjust, { id }),
    templates: () => API_ROUTE_PATTERNS.admin.templates,
    config: () => API_ROUTE_PATTERNS.admin.config,
    capabilities: () => API_ROUTE_PATTERNS.admin.capabilities,
    runtime: () => API_ROUTE_PATTERNS.admin.runtime,
    logs: () => API_ROUTE_PATTERNS.admin.logs,
    liveRooms: Object.freeze({
      list: () => API_ROUTE_PATTERNS.admin.liveRooms.list,
      detail: (roomNo) => buildPath(API_ROUTE_PATTERNS.admin.liveRooms.detail, { roomNo }),
      action: (roomNo) => buildPath(API_ROUTE_PATTERNS.admin.liveRooms.action, { roomNo }),
      occupantRemove: (roomNo, occupantId) =>
        buildPath(API_ROUTE_PATTERNS.admin.liveRooms.occupantRemove, { roomNo, occupantId })
    })
  })
});

const SOCKET_EVENTS = Object.freeze({
  room: Object.freeze({
    subscribe: "room:subscribe",
    ready: "room:ready",
    addBot: "room:add-bot",
    bid: "game:bid",
    play: "game:play",
    pass: "game:pass",
    trustee: "game:trustee",
    chat: "room:chat",
    update: "room:update",
    error: "room:error"
  }),
  party: Object.freeze({
    subscribe: "party:subscribe",
    ready: "party:ready",
    addBot: "party:add-bot",
    message: "party:message",
    action: "party:action",
    update: "party:update",
    error: "party:error",
    voicePeers: "voice:peers",
    voiceUserJoined: "voice:user-joined",
    voiceUserLeft: "voice:user-left"
  }),
  board: Object.freeze({
    subscribe: "board:subscribe",
    ready: "board:ready",
    addBot: "board:add-bot",
    move: "board:move",
    update: "board:update",
    error: "board:error"
  }),
  pickred: Object.freeze({
    subscribe: "pickred:subscribe",
    ready: "pickred:ready",
    draw: "pickred:draw",
    pickDiscard: "pickred:pick-discard",
    match: "pickred:match",
    discard: "pickred:discard",
    chat: "pickred:chat",
    update: "pickred:update",
    error: "pickred:error"
  }),
  bigtwo: Object.freeze({
    subscribe: "bigtwo:subscribe",
    ready: "bigtwo:ready",
    playHand: "bigtwo:play-hand",
    pass: "bigtwo:pass",
    chat: "bigtwo:chat",
    update: "bigtwo:update",
    error: "bigtwo:error"
  }),
  mahjong: Object.freeze({
    subscribe: "mahjong:subscribe",
    ready: "mahjong:ready",
    draw: "mahjong:draw",
    discard: "mahjong:discard",
    claim: "mahjong:claim",
    passClaim: "mahjong:pass-claim",
    chat: "mahjong:chat",
    update: "mahjong:update",
    error: "mahjong:error"
  }),
  racing: Object.freeze({
    subscribe: "racing:subscribe",
    ready: "racing:ready",
    input: "racing:input",
    update: "racing:update",
    error: "racing:error",
    chat: "racing:chat"
  }),
  fighting: Object.freeze({
    subscribe: "fighting:subscribe",
    ready: "fighting:ready",
    input: "fighting:input",
    update: "fighting:update",
    error: "fighting:error",
    chat: "fighting:chat"
  }),
  voice: Object.freeze({
    join: "voice:join",
    leave: "voice:leave",
    state: "voice:state",
    signal: "voice:signal",
    report: "voice:report"
  })
});

module.exports = {
  AUTH_SCOPES,
  PRESENCE_STATES,
  RECOVERY_SCOPES,
  VOICE_TRANSPORT_MODES,
  VOICE_REPORT_REASONS,
  LAUNCH_CONTRACT_KINDS,
  API_ROUTE_PATTERNS,
  API_ROUTES,
  SOCKET_EVENTS,
  buildPath,
  buildPresenceState,
  buildSeatRecoveryState,
  buildSessionRecoveryState,
  createHandlerContract,
  getContractAuthScope,
  normalizeReconnectGraceEndsAt
};
