...
class BoardRoomManager {
  ...
  startGomoku(room) {
    ...
    pushFeed(room, "棋盘展开，黑棋先手。", "system");
  ...
  finishMatch(room, result) {
    ...
    const settlements = buildStandardSettlements(room.players, {
      ...
      loserPenalty: room.gameKey === "gomoku" ? 40 : 30,
      winRank: room.gameKey === "gomoku" ? 14 : 12,
      lossRank: room.gameKey === "gomoku" ? -8 : -6
    });
  ...
  emitRoom(room) {
    for (const player of room.players) {
      for (const socketId of player.socketIds) {
        this.io?.to(socketId).emit("board:update", {
...
function getBoardBotName(gameKey, seatIndex, existingPlayers = []) {
  const pool = gameKey === "gomoku" ? GOMOKU_BOT_NAMES : CHINESE_CHECKERS_BOT_NAMES;
